/**
  ...
  - CGPA CALCULATOR LOGIC - Fixed & Optimized */ (function () { "use strict";

var  = SM.,  = SM., round = SM.round, clamp = SM.clamp, uid = SM.uid, esc =
SM.esc, store = SM.store;

var KEY = "sm_cgpa_rows"; var rows = store.get(KEY, []);

function classify(g) { if (g >= 3.7) return "Excellent standing"; if (g >= 3.3)
return "Very good"; if (g >= 3.0) return "Good"; if (g >= 2.0) return
"Satisfactory"; if (g > 0) return "Needs improvement"; return ""; }

if (!rows.length) { rows = [ { id: uid(), name: "Semester 1", gpa: 3.50,
credits: 15 }, { id: uid(), name: "Semester 2", gpa: 3.60, credits: 15 } ]; }

function render() { var container = $("#rows"); if (!container) return;
...
function attachEvents() { (".crow").forEach(function (row) { var id =
row.getAttribute("data-id"); var inputs = ("input", row);
...
function compute() { var totalCredits = 0, totalQualityPoints = 0;
...
function addSem() { rows.push({ id: uid(), name: "Semester " + (rows.length
+ 1), gpa: 4.0, credits: 15 }); save(); render(); SM.toast("Semester added",
"success"); }

document.addEventListener("DOMContentLoaded", function () { var addBtn1 =
$("#addRow"); var addBtn2 = $("#addRow2"); var clearBtn = $("#clearAll"); var
shareBtn = $("#shareBtn");
